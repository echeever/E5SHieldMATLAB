...
%                 s = [s 'For more info use: " >> explainError"'];
        ...
        function pinMode(a,pin,str)
            ...
            if nargin == 3,
                ...
                fwrite(a.aser,['rrP' '0'+pin '0'+val],'uchar');
        ...
        function digitalWrite(a,pin,st)
            ...
            fwrite(a.aser,['rrD' '0'+pin '0'+st],'uchar');
        end % digitalwrite
        ...
        function analogWrite(a,pin,val)
            ...
            % send mode, pin and value
            fwrite(a.aser,['rrA' '0'+pin val],'uchar');
        end % analogwrite
        ...
        function servoWrite(a,n,val)
            ...
            if a.chkp
                ...
                for i=1:V
                    value=val(i);
                    errstr=E5Shield.checknum(value,'value',0:2000);
                    if ~isempty(errstr), error(errstr); end
                end
            end
            ...
            if isscalar(val),
                for i=1:N
                    [hi, lo] = E5Shield.getHiLo(val);
                    fwrite(a.aser,['rrS' '0'+n(i) hi lo],'uchar');
                end
                if N==length(val),
                    for i=1:N
                        [hi, lo] = E5Shield.getHiLo(val(i));
                        fwrite(a.aser,['rrS' '0'+n(i) hi lo],'uchar');
                    end
                else
        ...
        function servoDisable(a,n)
            ...
            % Given Servo is disabled by setting its value to 2001.
            [hi, lo] = E5Shield.getHiLo(2001);
            for i=1:length(n)
                fwrite(a.aser,['rrS' '0'+n(i) hi lo],'uchar');
            end
        end %servoDisable
        ...
        function nunch=nunchuck(a)
            ...
            % send mode
            fwrite(a.aser,['rrn',1],'uchar');
            % get values
            fscanf(a.aser,'%d');  % First read can give bad readings
            fwrite(a.aser,['rrn',1],'uchar');
        ...
        function statLED(a,val)
            ...
            if a.chkp
                ...
                % check LED value, val
                errstr=E5Shield.checknum(val,'LED value',0:1);
                if ~isempty(errstr), error(errstr); end
            end
            %%%%%%%%%%%%%%%%%% PERFORM STATLED OUTPUT %%%%%%%%%%%%%%%%%%%%%
            if val==0
                fwrite(a.aser,['rrS800'],'uchar');
            else
                fwrite(a.aser,['rrS811'],'uchar');
            end
        end %statLED
    end % methods
    ...
    methods (Static) % static methods
        ...
        function [hi, lo] = getHiLo(x)
            hi = floor(x/32);   % if x is 0...2000, hi=0...62 (6 bits)
            lo = x-hi*32;       % This is remainder (0...31 - 5 bits)
            hi = hi + '0';      % Add to space (to send over ascii).
            lo = lo + '0';  
        end % getHiLo
